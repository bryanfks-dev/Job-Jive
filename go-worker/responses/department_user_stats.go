...
func (department_user_stats_response DepartmentUserStatsResponse) GetBestDepartmentUserStats(curr_date time.Time, department_id int) ([]DepartmentUserStatsResponse, error) {
	days_count := int(curr_date.Sub(curr_date.AddDate(0, -3, 0)).Hours() / 24)

	stmt := "WITH UserAttendances AS (SELECT u.User_ID, u.Full_Name, SUM(a.Type = 'Check-Out') att FROM `users` u LEFT JOIN `attendances` a ON a.User_ID = u.User_ID WHERE a.Date_Time >= DATE_SUB(?, INTERVAL 3 MONTH) AND u.Department_ID = ?) SELECT * FROM UserAttendances ua ORDER BY ua.att, ua.Full_Name DESC LIMIT 3"

	row, err := db.Conn.Query(stmt, curr_date.Format(time.DateOnly), department_id)
...
func (department_user_stats_response DepartmentUserStatsResponse) GetWorstDepartmentUserStats(curr_date time.Time, department_id int) ([]DepartmentUserStatsResponse, error) {
	days_count := int(curr_date.Sub(curr_date.AddDate(0, -3, 0)).Hours() / 24)

	stmt := "WITH UserAttendances AS (SELECT u.User_ID, u.Full_Name, SUM(a.Type = 'Check-Out') att FROM `users` u LEFT JOIN `attendances` a ON a.User_ID = u.User_ID WHERE a.Date_Time >= DATE_SUB(?, INTERVAL 3 MONTH) AND u.Department_ID = ?), BestUserAttendances AS (SELECT * FROM UserAttendances ua ORDER BY ua.att, ua.Full_Name DESC LIMIT 3) SELECT ua.* FROM UserAttendances ua, BestUserAttendances bua WHERE ua.User_ID NOT IN (bua.User_ID) ORDER BY ua.att, ua.Full_Name ASC LIMIT 3"
