<div
    ...
    <time class="text-lg font-semibold text-gray-900 dark:text-white">{{ date('l, d F Y', strtotime($record['date']))
        }}</time>
    <ol class="flex flex-wrap mt-3 text-gray-900 md:gap-6 dark:text-white">
        <li class="items-center block p-3 sm:flex">
            ...
            <div class="flex flex-col">
                ...
                <span class="text-xs font-medium text-blue-600 dark:text-blue-500">{{ $record['check_in_time'] ?? '-'
                    }}</span>
            </div>
        </li>
        <li class="items-center block p-3 sm:flex">
            ...
            <div class="flex flex-col">
                ...
                <span class="text-xs font-medium text-blue-600 dark:text-blue-500">{{ $record['check_out_time'] ?? '-'
                    }}</span>
            </div>
        </li>
    </ol>
    ...
    <form method="POST" action="{{ route('user.peoples.update-attendance', ['id' => $employee]) }}">
        @csrf
        <ol class="flex flex-wrap pt-4 mt-3 text-gray-900 md:gap-6 dark:text-white">
            ...
            <input type="hidden" id="check_in_time" name="check_in_time" value="{{ date('Y-m-d ', strtotime($record['date'])) . $configs['check_in_time'] . ':00'
    }}">

            <input type="hidden" id="check_out_time" name="check_out_time" value="{{ date('Y-m-d ', strtotime($record['date'])) . $configs['check_out_time'] . ':00'
    }}">
            <button type="submit"
